...
# Test lots of variations on a task
...
# Pass in tasks as tuples - to see how multiple test parameters work
...
# Move the task list to a variable outside the function
tasks_to_try = (Task('sleep', done=True),
                Task('wake', 'brian'),
                Task('wake', 'brian'),
                Task('breate', 'BRIAN', True),
                Task('exercise', 'BrIaN', False))
...
# To make the test output slightly more readable
...
# The same thing can be done on tasks
@pytest.mark.parametrize('task', tasks_to_try, ids=task_ids)
class TestAdd():
    """Demonstrate parametrize and test classes"""

    def test_equivalent(self, task):
        """Similar test, just in a classe"""
        task_id = tasks.add(task)
        t_from_db = tasks.get(task_id)
        assert equivalent(t_from_db, task)
